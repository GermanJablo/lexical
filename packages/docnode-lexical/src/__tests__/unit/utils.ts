...
import {boolean, number, string, UndoManager} from 'docnode';
import {expect} from 'vitest';

const ULID_REGEX = /^[\da-hjkmnp-tv-zA-HJKMNP-TV-Z]{26}$/;
...
function getStateSnapshot(doc: Doc, isTestNode = true): unknown[] {
  ...
  doc.root.descendants().forEach((node, deepLevel) => {
    ...
    if (node.prev) {
      ...
      expect(node.parent.first).toBe(node);
    }
    if (node.next) {
      expect(node.next.prev).toBe(node);
    } else if (node.parent) {
      expect(node.parent.last).toBe(node);
    }
    if (node.first) {
      expect(node.first.parent).toBe(node);
    }
    if (node.last) {
      expect(node.last.parent).toBe(node);
    }
    if (isTestNode) {
...
export function init(
  ...
  checkUndoManager(1, doc, () => {
    ...
    fn({doc, node1, node2, node3, node4, root});
  });
}
...
export function checkUndoManager(
  ...
  listen(doc, callback, (changeEvent) => {
    ...
    allOperations.forEach((op) => {
      switch (op[0]) {
        case 0: {
          ...
          if (parent !== 0) {
            expect(parent).toMatch(DOCNODE_ID());
          }
          if (prev !== 0) {
            expect(prev).toMatch(DOCNODE_ID());
          }
          if (next !== 0) {
            expect(next).toMatch(DOCNODE_ID());
          }
          break;
        }
        case 1: {
    ...
    for (const id in operations[1]) {
      if (!diff.inserted.has(id)) {
        expect(diff.updated.has(id)).toBe(true);
      }
    }
    for (const id in inverseOperations[1]) {
      if (!diff.deleted.has(id)) {
        expect(diff.updated.has(id)).toBe(true);
      }
    }
    ...
    doc._nodeMap.forEach((node) => {
      ...
      if (!node.is(Text) || diff.updated.has(node.id)) {
        return;
      }
  ...
  for (const [i, changeEvent] of Array.from(changeEvents.entries())) {
    updateAndListen(
      doc3,
      () => {
